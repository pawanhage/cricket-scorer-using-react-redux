...
function SetWhoOut() {
    ...
    return (
        <>
            ...
            <div>
                <Dropdown disabled={!wicketDetails.wicketType || [BOWLED, LBW, HIT_WICKET, STUMPED].includes(wicketDetails.wicketType)} style={{ width: "100%" }} optionLabel="name" value={wicketDetails.whoOut} options={currentBatsmanOptions} onChange={(e) => dispatchWicketDetails({ type: 'SET_WHO_OUT', payload: e.value })} placeholder="Select" />
            </div>
        </>
    )
}
...
function SetOutByPlayer() {
    ...
    return (
        <>
            ...
            <div>
                <Dropdown disabled={!wicketDetails.wicketType || [BOWLED, LBW, HIT_WICKET, FIELD_OBSTRUCT].includes(wicketDetails.wicketType)} style={{ width: "100%" }} optionLabel="name" value={wicketDetails.outByPlayer} options={bowlingTeamPlayersOptions} onChange={(e) => dispatchWicketDetails({ type: 'SET_OUT_BY_PLAYER', payload: e.value })} placeholder="Select" />
            </div>
        </>
    )
}
